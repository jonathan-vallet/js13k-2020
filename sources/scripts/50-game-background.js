...
function initGameBackground() {
    ...
    for(let x = 0; x <= gameCanvas.width; x += getRandomNumber(gameCanvas.width / 7, gameCanvas.width / 9)) {
        let sandY = getRandomNumber(190, 210);
...
function drawBackground() {
    ...
    ctx.fillStyle = '#cee';
    ctx.beginPath();
    ctx.rect(0, 0, gameCanvas.width, 200);
    ctx.fill();

    // sun
    ctx.fillStyle = '#ffa';
    ctx.beginPath();
    ctx.arc(gameCanvas.width - 400, 200, 180, PI, 2 * PI);
    ctx.fill();
...
function drawBackgroundShape(ctx, fillStyle, coordinateList, offset = 0, min, max) {
    ctx.beginPath();
    ctx.fillStyle = fillStyle;
    ctx.moveTo(0, gameCanvas.height);
    ctx.lineTo(0, gameCanvas.height - offset);
    coordinateList.forEach(coordinate => {
        coordinate.y += coordinate.direction * 0.2;
